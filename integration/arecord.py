...
class SharedState:
    def __init__(self):
        ...
        self.lidar_min_mm = None          # float | None
        self.yolo_labels = set()          # set[str]
        self.last_alert_ts = 0.0          # throttle alerts
...
def record_wav_arecord() -> str:
    ...
    # no silenciar stderr si quieres ver fallos reales:
...
def lidar_thread(shared: SharedState):
    ...
    max_mm = float(CONFIG["lidar"].get("max_considered_mm", 2000))  # solo para descartar basura
...
# --------------------------
# MAIN: YOLO loop + alert trigger
# --------------------------
def main():
    print("[Main] Iniciando sistema mínimo...")
    ...
    # Whisper init (solo se usa al presionar botón)
    try:
        import whisper
    ...
    def on_button():
        ...
        if whisper_model is None:
            print("[BOTON] Whisper no está disponible, no puedo transcribir.")
            return
        ...
        try:
            print("[BOTON] Transcribiendo con Whisper...")
        ...
        if not text:
            print("[BOTON] Transcripción vacía.")
            return
    ...
    try:
        while True:
            ...
            for i in range(len(boxes)):
                ...
                seen.add(str(labels_map.get(cls, cls)))
            ...
            if (now - last_console) >= console_every:
                print(f"[Estado] LiDAR_min_mm={int(dmm) if dmm is not None else None} | YOLO={sorted(seen)}")
                last_console = now

            # ALERTA automática si LiDAR < 1m
            if (dmm is not None) and (dmm < alert_mm) and shared.can_alert(alert_min_interval):
                yolo_seen = shared.get_yolo_labels()
                prompt = (
                    f"ALERTA: objeto a {int(dmm)} mm al frente.\n"
                    f"Etiquetas YOLO visibles: {', '.join(yolo_seen) if yolo_seen else '(ninguna)'}.\n"
                    f"Da una recomendación inmediata en una frase."
                )
                resp = ask_ollama(prompt)
                print("[ALERTA->Ollama]", resp)

            # (Opcional) mostrar ventana para debug visual
            if CONFIG["sistema"].get("show_window", True):
                cv2.imshow("Sistema Minimo", frame)
                if cv2.waitKey(1) == ord("q"):
                    break
            else:
                # sin ventana, evitar 100% CPU
                time.sleep(0.01)
