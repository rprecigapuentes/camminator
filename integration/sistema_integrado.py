...
class SharedState:
    def __init__(self):
        ...
        self._data = {
            ...
            "detections": {},
            # Obeto detectados por el LIDAR: {'objeto_id': {'angle': a, 'distance': d}}
            "lidar_objects": {},
            ...
            "current_speech_action": None,  # Puede ser 'proximity_alert', 'llm_response', o None
    ...
    def set_proximity_alert(self, alert_data):
        with self._lock:
            ...
            self._data["stop_speech_flag"].set() # Dispara la interrupción
...
def lidar_process(shared_state):
    ...
    lidar = PyRPLidar()
    try:
        lidar.connect(port=CONFIG["lidar"]["port"], baudrate=CONFIG["lidar"]["baudrate"], timeout=3)
    ...
    while True:
        try:
            ...
            for scan in scan_generator:
                ...
                in_front_sector = ((CONFIG["lidar"]["front_angle_min1"] <= angle_norm <= CONFIG["lidar"]["front_angle_max1"]) or 
                                   (CONFIG["lidar"]["front_angle_min2"] <= angle_norm <= CONFIG["lidar"]["front_angle_max2"]))

                if in_front_sector and distance <= CONFIG["lidar"]["alert_distance"]:
                    if not current_object:
                        current_object = {
                            'min_angle': angle_norm, 'max_angle': angle_norm,
                            'min_distance': distance, 'max_distance': distance,
                            'avg_distance': distance, 'count': 1
                        }
                        ...
                        if angle_diff <= CONFIG["lidar"]["angle_gap_threshold"] and distance_diff <= CONFIG["lidar"]["distance_threshold"]:
            ...
            time.sleep(0.1) # Escanea a ~10 Hz
...
def audio_llm_process(shared_state, button):
    ...
    def record_audio():
        ...
        frames = []
        for _ in range(0, int(CONFIG["asistente"]["audio_rate"] / CONFIG["asistente"]["audio_chunk"] * CONFIG["asistente"]["record_seconds"])):
            data = stream.read(CONFIG["asistente"]["audio_chunk"])
            frames.append(data)
...
def main():
    ...
    labels = model.names
    cap = cv2.VideoCapture(CONFIG["yolo"]["camera_source"])
    if CONFIG["yolo"]["resolution"]:
    ...
    while True:
        ...
        for i in range(len(detections)):
            ...
            if conf > CONFIG["yolo"]["threshold"]:
                ...
                lidar_objects = shared_state.get("lidar_objects")
                min_distance = float('inf')
                for obj_data in lidar_objects.values():
                    if abs(angle_deg - obj_data['angle']) < 20: # Umbral de ángulo para asociar
                ...
                detected_objects[classname] = {
                    ...
                    'angle': angle_deg,
                    'distance': min_distance / 1000.0 # Convertir mm a metros
                }
        ...
        for obj_name, data in detected_objects.items():
            if data['distance'] < CONFIG["sistema"]["min_proximity_distance"] and data['distance'] > 0:
                ...
                break # Solo una alerta a la vez
        ...
        cv2.putText(frame, f'FPS: {avg_frame_rate:.2f}', (10, frame_h - 20), cv2.FONT_HERSHEY_SIMPLEX, .7, (0,255,255), 2)
...
if __name__ == '__main__':
    try:
        ...
        # Limpieza de recursos (la cámara se libera en el main loop)
